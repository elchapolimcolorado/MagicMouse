    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
				...
				if (HidD_GetPreparsedData(m_hHandle, out lpData))	// get windows to read the device data into an internal buffer
				{
                    try
                    {
                       ...
                       // m_oFile = new FileStream(new SafeFileHandle(m_hHandle, false), FileAccess.Read | FileAccess.Write, m_nInputReportLength, true);
                        ...
                        Thread read_usb = new Thread(BeginRead);
                        read_usb.Name = "READ_USB";
                        read_usb.Priority = ThreadPriority.AboveNormal;
                        read_usb.Start();
                        //Task kkk = Task.Factory.StartNew(() => BeginAsyncRead());
        ...
        private void BeginRead()
        {
            while (true)
            {
               ...
               try
                {
                    int len_buf = m_oFile.Read(arrInputReport, 0, m_nInputReportLength);
                    if (len_buf > 0)
                    {
                        HandleDataReceived(arrInputReport);
                    }
                }
      ...
      /*  protected void ReadCompleted(IAsyncResult iResult)
        ...
        protected virtual void HandleDataReceived(byte[] arrInputReport)
		{
